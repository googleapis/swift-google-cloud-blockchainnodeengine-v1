...
extension Clients {
  ...
  class BlockchainNodeEngineTransport: BlockchainNodeEngineStub {
    let inner: GoogleCloudGax.HTTPClient

    public init(_ options: GoogleCloudGax.ClientOptions = .init()) throws {
      self.inner = try GoogleCloudGax.HTTPClient(
        from: options, withDefaultEndpoint: "https://blockchainnodeengine.googleapis.com")
    }
    ...
    public func listBlockchainNodes(
      ...
      let path = try { () throws -> Swift.String in
        ...
        return "/v1/\(pathVariable0)/blockchainNodes"
      }()
      var query = [
    ...
    public func updateBlockchainNode(
      ...
      let path = try { () throws -> Swift.String in
        guard let pathVariable0 = request.blockchainNode.map({ $0.name }), !pathVariable0.isEmpty
        else {
          throw GoogleCloudGax.RequestError.binding(
            "'request.blockchain_node.name' is not set or is empty")
        }
        return "/v1/\(pathVariable0)"
      }()
      var query = [
